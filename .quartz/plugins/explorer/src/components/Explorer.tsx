...
const defaultOptions: ExplorerOptions = {
  ...
  sortFn: (a: FileTrieNode, b: FileTrieNode) => {
    const getLeadingNumber = (node: FileTrieNode): number | undefined => {

      let name = node.slugSegment ?? ""

      if (node.data && typeof node.data.filePath === "string") {
        const parts = node.data.filePath.split("/")
        const last = parts[parts.length - 1] ?? ""
        if (last.toLowerCase() === "index.md" && parts.length >= 2) {
    ...
    const orderA = getLeadingNumber(a)
    const orderB = getLeadingNumber(b)
